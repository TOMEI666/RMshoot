...
Classifier::Classifier(const std::string & config_path)
{
  ...
  try {
    ...
    tools::logger()->info("[Classifier] OpenCV DNN loaded ONNX model successfully");
  } catch (const cv::Exception & e) {
    tools::logger()->warn("[Classifier] OpenCV DNN failed to load ONNX model: {}, falling back to OpenVINO", e.what());
    net_ = cv::dnn::Net();  // Create empty net
  }

  auto ovmodel = core_.read_model(model);
  compiled_model_ = core_.compile_model(
    ovmodel, "AUTO", ov::hint::performance_mode(ov::hint::PerformanceMode::LATENCY));
  tools::logger()->info("[Classifier] OpenVINO loaded model successfully");
}
...
void Classifier::classify(Armor & armor)
{
  ...
  // Try OpenCV DNN first, fallback to OpenVINO if DNN failed to load
  if (!net_.empty()) {
    cv::Mat gray;
