...
void MindVision::set_exposure(double exposure_ms)
{
  exposure_ms_ = exposure_ms;
  if (handle_ >= 0) {
    CameraSetExposureTime(handle_, exposure_ms_ * 1e3);  // 转换为微秒
    tools::logger()->info("[MindVision] Exposure set to {:.1f} ms ({:.0f} us)", exposure_ms, exposure_ms_ * 1e3);
  }
}
